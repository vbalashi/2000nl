...
### Sentence TTS Audio Playback on Localhost
**Added:** 2026-02-06 (Sprint: Bug Fix Blitz)
**User Story:** US-062.1

**Behavior:**
Sentence TTS audio generated by `/api/tts` is now cached under the UI `public/audio/tts` directory (or `TTS_CACHE_DIR`) and returned as a relative `/audio/tts/<key>.mp3` URL, so playback works on `http://localhost:3000` without 404s. This fixes a dev-only mismatch where sentence audio could be written outside the Next.js-served `public/` tree depending on where the dev server was launched from.
