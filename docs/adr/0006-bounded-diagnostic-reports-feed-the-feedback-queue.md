...
The learner sees only transient delivery status. Internal admin reviewers decide whether a Feedback Item warrants a manually written GitHub issue, and accepted reports are removed from the local outbox immediately. This boundary deliberately favors bounded, classifiable evidence over general-purpose debugging capture so forbidden browser and credential sources are structurally unreachable from automatic technical collection and diagnostics cannot grow into a performance-sensitive surveillance channel.
