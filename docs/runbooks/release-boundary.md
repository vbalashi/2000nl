...
The `release-boundary` check runs for every PR to `main` from the trusted base-branch workflow. For a PR whose files match the deployment workflow's path filter, the PR body must contain a line `Release-Ready: yes`. Set this only after verifying that the PR can deploy without another PR or coordinated step. The check also blocks a PR while another open PR targets its head branch. Documentation-only PRs do not need the declaration.

For changes that require several PRs, keep the component PRs open for review and create a single integration PR to `main` with the complete tested result. The integration PR needs its own `Release-Ready: yes` statement. Do not merge a parent PR into `main` merely because its tests pass while a child is still required. If a child PR is opened after the parent's check passed, rerun the check or update the parent PR body before merging; GitHub does not automatically rerun an existing PR check when an unrelated child PR opens.
