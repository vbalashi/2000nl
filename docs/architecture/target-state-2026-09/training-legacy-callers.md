...
| `TrainingCard*.test.tsx`, `trainingCardPresentation.test.ts`, legacy-mode Screen tests | Test consumers, including explicit V2-off configuration, not external product consumers. | Keep only V2 transition/loading coverage; re-baseline the remaining legacy-only cases separately. | Renderer-only files deleted; TrainingScreen suite passes with 38 active tests and 15 explicitly marked legacy/re-baseline cases (65 active across TrainingScreen + controller suites). |
