...
| `lib/training/selectionService.ts` / scheduler RPC overloads | Both next-card and prefetch select `get_next_card` or `get_next_filtered_card`; session-owned turns select `get_next_training_session_card`. | #353 retains the public direct RPCs as stable compatibility entry points and makes the private v2 candidate relation canonical. #355 also keeps the immediately preceding public shapes as adapters for cached browser bundles. | No public signature is removed without a caller audit **and** a staged-client deprecation plan; private candidate v1 overloads are deleted. |
...
| --- | --- | --- |
| `selectionService.ts` before a finite session and after a scope/filter change | Direct `get_next_card` / `get_next_filtered_card` selection, including its daily-cap policy and explicit future-practice switch. | The public RPCs remain compatible entry points and call private v2 directly; the immediately previous public signatures adapt to them with practice disabled. Any public removal requires a new external-client audit and a staged deprecation plan that covers cached browser bundles. |
