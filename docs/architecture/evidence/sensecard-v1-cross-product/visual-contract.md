...
| `10.16.01–02` | APPROVED baseline | multi-sense Full/Narrow anatomy | every meaning owns its disclosure, translation, exposure and learning state; later decision removes aggregate `Betekenissen` chrome |
| `20.70` | HISTORICAL REFERENCE · PARTIALLY SUPERSEDED | Full multi-sense geometry | retain the border-cutout ordinal and per-sense control geometry; do not copy its old Meaning(s), grading or footer chrome |
| `20.71` | HISTORICAL REFERENCE · PARTIALLY SUPERSEDED | Narrow and long-headword geometry | retain pronunciation-boundary wrapping and container-responsive type; do not copy its old Meaning(s), grading or footer chrome |
...
- `SenseCardHeadwordLockup`: metadata above; article and headword share one
  baseline; audio follows the headword; long type scales with container width.
...
- Library per-sense controls: exposure, translation and collapse at top-right.
  Clicking a collapsed card expands it; the explicit arrow only collapses.
