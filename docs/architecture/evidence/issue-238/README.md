...
| index + count-only helper + materialized learner sets/settings | 1,976–1,980 | pass |

The index-only experiment showed that a narrow scope was necessary but not
sufficient. The first helper then exposed an inlined `user_settings` lookup once
per candidate; making limits and learner sets explicit one-time relations
removed that amplification. The final representative RPC executed in 16.789 ms
and stayed below both the 4,000-block characterization budget and unchanged
2,000-ms production gate timeout. Its counts match the migration-126 selector
for `both`, `new`, `review`, and multi-mode cases. List and filtered requests
continue through the authoritative selector fallback.
