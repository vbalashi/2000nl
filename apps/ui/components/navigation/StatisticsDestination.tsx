...
export function StatisticsDestination({
  ...
  const progress = total > 0
    ...
    : 0;
  const metrics = [
    { label: text.newToday, value: `${stats.newWordsToday} / ${stats.dailyNewLimit}` },
    { label: text.reviewedToday, value: String(stats.reviewCardsDone) },
    { label: text.dueNow, value: String(stats.reviewCardsDue) },
    { label: text.learned, value: `${stats.totalWordsLearned} / ${stats.totalWordsInList}` },
  ];
    ...
    >
      <header className="relative z-20 grid flex-none grid-cols-[1fr_auto_1fr] items-center border-b border-slate-200 bg-white/90 px-3 py-2 shadow-sm backdrop-blur md:px-6 md:py-3 dark:border-slate-800 dark:bg-slate-900/80">
        ...
        <div className="hidden justify-self-center sm:block">
          <AppDestinationNav active="statistics" interfaceLanguage={interfaceLanguage} onNavigate={onNavigate} />
        </div>
        <button
          type="button"
      ...
      <main className="scrollbar-hide min-h-0 flex-1 overflow-y-auto px-4 py-6 sm:px-6 md:px-8">
        <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="mt-7 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
            {metrics.map((metric) => (
              <section key={metric.label} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:p-5 dark:border-slate-800 dark:bg-slate-900">
                <p className="text-[10px] font-semibold uppercase tracking-[0.14em] text-slate-500 sm:text-xs sm:tracking-[0.16em] dark:text-slate-400">{metric.label}</p>
                <p className="mt-3 text-2xl font-bold text-slate-950 sm:mt-4 sm:text-3xl dark:text-white">{metric.value}</p>
              </section>
            ))}
          </div>
