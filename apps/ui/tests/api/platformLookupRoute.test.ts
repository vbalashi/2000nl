...
describe("/api/platform/lookup", () => {
  ...
  test.each([
    ...
    },
    {
      ...
      expected: {
        ...
        message: "provider_unknown_error:a916ee424eb2d8d4630b04a1",
        },
      },
    },
  ...
  test("attaches cached generated-entry translations to legacy generated sections", async () => {
    const { POST } = await import("@/app/api/platform/lookup/route");
    getUser.mockResolvedValueOnce({
      data: { user: { id: "user-1" } },
      error: null,
          ...
          data: [
            {
              ...
              overlay: {
                ...
                meanings: [
                  {
                    definition: "люди, изучающие Вселенную",
                    examples: ["Астрономы используют телескопы."],
                    context: "множественное число от астроном",
                  },
                ],
              },
    ...
    expect(response.status).toBe(200);
    const payload = await response.json();
    expect(payload.items[0].entry.content.headwordTranslation).toBe("астрономы");
    expect(payload.items[0].entry.content.summary).toEqual({
      definition: "Mensen die het heelal bestuderen.",
      definitionTranslation: "люди, изучающие Вселенную",
      example: "Astronomen gebruiken telescopen.",
      exampleTranslation: "Астрономы используют телескопы.",
    });
    expect(payload.items[0].entry.content.sections).toEqual([
      {
        id: "meaning-1",
        ...
        kind: "meaning",
        text: "Mensen die het heelal bestuderen.",
        translation: "люди, изучающие Вселенную",
      },
      {
        id: "example-1-1",
        sourcePath: "raw.example.source",
        kind: "example",
        text: "Astronomen gebruiken telescopen.",
        translation: "Астрономы используют телескопы.",
      },
      {
        id: "note-1",
        sourcePath: "raw.notes",
        kind: "note",
        text: "Meervoud van astronoom.",
        translation: "множественное число от астроном",
      },
    ]);
  });
