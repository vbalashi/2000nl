...
describe("NUC database contract deployment", () => {
  ...
  test("postflight protects the current canonical scheduler and sequential-introduction seam", () => {
    ...
    const postflight =
      ...
      read("db/deploy-contract/postflight-172.sql") +
      read("db/deploy-contract/postflight-171.sql") +
      read("db/deploy-contract/postflight-170.sql") +
      read("db/deploy-contract/postflight-168.sql") +
      read("db/deploy-contract/postflight-167.sql") +
      read("db/deploy-contract/postflight-166.sql") +
      read("db/deploy-contract/postflight-165.sql") +
      read("db/deploy-contract/postflight-163.sql") +
      read("db/deploy-contract/postflight-162.sql") +
      read("db/deploy-contract/postflight-161.sql") +
      read("db/deploy-contract/postflight-160.sql") +
      read("db/deploy-contract/postflight-159.sql") +
      read("db/deploy-contract/postflight-158.sql") +
    ...
    expect(postflight).toContain("user-training-scopes-table-grants");
    expect(postflight).toContain("user-training-scopes-rpc-boundary");
    ...
    expect(postflight).toContain("scoped-ordinary-predecessors");
    expect(postflight).toContain("training-dictionary-material");
    expect(postflight).toContain("extra-source-scope");
    expect(postflight).toContain("content-child-lookup");
    expect(postflight).toContain("bounded-extra-eligibility");
    ...
    expect(driftWorkflow).toContain("-f db/deploy-contract/postflight-173.sql");
  });
