-- Adjust detailed training stats to:
-- 1) Count due items as "due today" (before midnight) instead of "due now".
-- 2) Count reviews done only if the next review is >24h away (graduated from today).
...
BEGIN
    ...
    -- Review cards done today: cards reviewed today AND next review is >24h away
    ...
    WHERE s.user_id = p_user_id
      ...
      AND s.next_review_at > now() + interval '1 day'
    ...
    -- Review words due: distinct words due today (before midnight), graduated + learning
    ...
    -- Review cards due: cards due today (before midnight), graduated + learning
