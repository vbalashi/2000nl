-- Fix NULL interval cards being skipped in queue selection
-- 
-- Problem: Cards with fsrs_last_interval = NULL were falling through all queue conditions:
-- - Learning queue checks: fsrs_last_interval < 1.0 -> NULL doesn't match
-- - Review queue checks: fsrs_last_interval >= 1.0 -> NULL doesn't match
--
-- Solution: Use COALESCE(fsrs_last_interval, 0) to treat NULL as 0 (new/learning cards)
...
BEGIN
    ...
    -- Count new cards introduced today (first time a word was seen in any enabled mode)
    ...
    -- Count learning cards due (interval < 1 day, or NULL which means new/learning)
    -- FIX: Use COALESCE to treat NULL interval as 0 (learning)
    ...
    WHERE s.user_id = p_user_id 
      ...
      AND COALESCE(s.fsrs_last_interval, 0) < 1.0  -- FIX: NULL treated as 0
    ...
    WHERE s.user_id = p_user_id 
      AND s.mode = ANY(p_modes)
      AND s.fsrs_last_interval >= 1.0  -- Graduated (interval >= 1 day) - NULL won't match, which is correct
    ...
    -- Priority A: If queue_turn = 'new' OR card_filter = 'new', try NEW_POOL first
    IF (p_queue_turn = 'new' OR p_card_filter = 'new') AND p_card_filter != 'review' THEN
        
        -- A1: First, pick learning cards that are due (interval < 1 day or NULL, due now)
        -- FIX: Use COALESCE to treat NULL interval as 0 (learning)
        IF v_learning_due_count > 0 THEN
        ...
        -- A2: If no learning cards due, pick a new unseen card (if quota allows)
    ...
    -- Priority B: If queue_turn = 'review' OR card_filter = 'review', try REVIEW_POOL
    ...
    IF v_word_id IS NULL AND p_queue_turn = 'auto' AND p_card_filter = 'both' THEN
        ...
        -- C1: First check learning cards due (including NULL interval = new cards seen once)
        -- FIX: Use COALESCE to treat NULL interval as 0 (learning)
        IF v_learning_due_count > 0 THEN
            SELECT s.word_id, s.mode, 'learning' INTO v_word_id, v_selected_mode, v_source
            FROM user_word_status s
        ...
        -- C2: Then check overdue reviews (graduated cards due now)
        ...
        -- C3: Then try new card if quota allows
        IF v_word_id IS NULL AND v_new_today < v_settings.daily_new_limit THEN
            v_selected_mode := p_modes[1 + floor(random() * array_length(p_modes, 1))::int];
    ...
    -- Fallback D: If still nothing, pick most forgotten review (even if not due yet)
    -- This is the only place where we allow selecting not-yet-due cards
...
-- Also update the scenario-based overload to use the fixed function
-- (It delegates to the main function which now has the fix)
