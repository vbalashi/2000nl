-- Keep grouped body search from returning non-token substring matches.
-- FTS remains the primary body matcher. The substring fallback is retained only
-- for known lexical forms at token boundaries, so short words such as "fel" can
-- match "felle" but not "tafel", "betwijfelen", "Felix", or "feliciteren".
...
BEGIN
    ...
    substring_page AS MATERIALIZED (
        ...
        FROM dictionary_search_fields f
        JOIN eligible_dictionaries ed ON ed.id = f.dictionary_id
        WHERE v_term_query IS NOT NULL
          AND length(v_query_unaccent) >= 3
          AND (
          ...
          AND f.normalized_text_unaccent ~ v_term_query
