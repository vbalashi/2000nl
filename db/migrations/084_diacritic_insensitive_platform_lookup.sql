-- Make platform lookup tolerant of missing Dutch diacritics.
--
-- AudioFilms and other connected clients often receive clicked text after
-- browser/client normalization. A lookup for "echt" should still find the
-- dictionary headword "écht".
...
BEGIN
    ...
    v_like_query_plain := CASE WHEN v_query IS NULL THEN NULL ELSE '%' || v_query || '%' END;
    ...
    candidate_matches AS (
        ...
        WHERE v_query IS NOT NULL
          AND (
            normalize_dictionary_search_text(headword) = v_query
            OR normalize_dictionary_search_text_unaccent(headword) = v_query_unaccent
          )
        ...
        WHERE v_query IS NOT NULL
          ...
          AND (
            normalize_dictionary_search_text(f.form) = v_query
        ...
        FROM visible_entries
        WHERE v_query IS NOT NULL
          AND normalize_dictionary_search_text_unaccent(headword) LIKE v_like_query
        ...
        WHERE v_query IS NOT NULL
          AND (
            lower(COALESCE(raw#>>'{example}', '')) LIKE v_like_query_plain
            OR lower(COALESCE(raw#>>'{meanings,0,example}', '')) LIKE v_like_query_plain
            OR lower(COALESCE(raw#>>'{meanings,0,examples}', '')) LIKE v_like_query_plain
            OR lower(COALESCE(raw#>>'{translation,text}', '')) LIKE v_like_query_plain
          )
        ...
        WHERE v_query IS NOT NULL
          AND (
            lower(COALESCE(raw#>>'{definition}', '')) LIKE v_like_query_plain
            OR lower(COALESCE(raw#>>'{meanings,0,definition}', '')) LIKE v_like_query_plain
            OR lower(COALESCE(raw#>>'{meanings,0,context}', '')) LIKE v_like_query_plain
            OR lower(COALESCE(raw#>>'{notes}', '')) LIKE v_like_query_plain
          )
        ...
        FROM visible_entries
        WHERE v_query IS NOT NULL
          AND lower(raw::text) LIKE v_like_query_plain
    ),
    ranked_matches AS (
    ...
    ranked_entries AS MATERIALIZED (
        SELECT
            ...
            CASE
                ...
                WHEN normalize_dictionary_search_text(v.headword) = v_query THEN 0
                WHEN normalize_dictionary_search_text_unaccent(v.headword) = v_query_unaccent THEN 0
                WHEN normalize_dictionary_search_text_unaccent(v.headword) LIKE v_query_unaccent || '%' THEN 1
                WHEN normalize_dictionary_search_text_unaccent(v.headword) LIKE v_like_query THEN 2
                ELSE 3
            END AS headword_rank,
            CASE
                WHEN v_raw_query IS NULL THEN 0
                WHEN v.headword = v_raw_query THEN 0
                WHEN normalize_dictionary_search_text(v.headword) = v_query THEN 1
                WHEN normalize_dictionary_search_text_unaccent(v.headword) = v_query_unaccent THEN 2
                ELSE 3
