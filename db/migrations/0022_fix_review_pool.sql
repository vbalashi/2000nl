-- Fix Review Pool Selection Bug
-- Priority B was selecting graduated cards even if not due yet.
-- This adds the missing `next_review_at <= now()` check to ensure
-- only actually due cards are selected from the review pool.
...
BEGIN
    ...
    -- Count words in NEW_POOL (unseen + learning with interval < 1 day)
    ...
    WHERE s.user_id = p_user_id 
      ...
      AND s.fsrs_last_interval < 1.0  -- Still in learning (interval < 1 day)
    ...
    -- Determine actual queue to use based on filter and queue_turn
    -- If card_filter is 'new', only use new pool
    -- If card_filter is 'review', only use review pool
    -- If card_filter is 'both', use queue_turn to decide
    ...
    IF (p_queue_turn = 'new' OR p_card_filter = 'new') AND p_card_filter != 'review' THEN
        ...
        -- A1: First, pick learning cards that are due (interval < 1 day, due now)
    ...
    -- FIX: Added next_review_at <= now() to only pick cards that are actually due
    IF v_word_id IS NULL AND (p_queue_turn = 'review' OR p_card_filter = 'review') AND p_card_filter != 'new' THEN
        ...
        WHERE s.user_id = p_user_id 
          ...
          AND s.next_review_at <= now()    -- FIX: Only select cards that are actually due
          AND (s.frozen_until IS NULL OR s.frozen_until <= now())
          AND s.hidden = false
          AND s.fsrs_enabled = true
          AND NOT (s.word_id = ANY(p_exclude_ids))
    ...
    IF v_word_id IS NULL AND p_queue_turn = 'auto' AND p_card_filter = 'both' THEN
        ...
        -- C1: First check if there are overdue reviews (graduated cards due now)
        SELECT s.word_id, s.mode, 'review' INTO v_word_id, v_selected_mode, v_source
        FROM user_word_status s
        JOIN word_entries w ON w.id = s.word_id
        WHERE s.user_id = p_user_id 
        ...
        -- C2: Then check learning cards due
...
-- Keep the backward-compatible single-mode overload unchanged
