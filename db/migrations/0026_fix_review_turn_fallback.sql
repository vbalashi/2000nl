-- Fix: when queue_turn='review' but no due reviews exist,
-- do NOT fall back to future-due reviews (Fallback D) if learning/new are available.
--
-- Runtime evidence (debug logs) showed queueTurn='review', reviewPoolSize=0,
-- yet a future-due review card was selected via Fallback D.
-- This patch adds a fallback for queueTurn='review' (with card_filter='both'):
-- 1) learning due
-- 2) new (if quota allows)
-- Only then allow future-due review practice.
...
BEGIN
    ...
    -- Learning due count (NULL interval treated as 0)
    ...
    -- Review pool count: graduated + due now
    ...
    IF v_word_id IS NULL AND (p_queue_turn = 'review' OR p_card_filter = 'review') AND p_card_filter != 'new' THEN
        ...
        -- NEW: If no due review exists, fall back to learning/new (only when card_filter='both').
        IF v_word_id IS NULL AND p_queue_turn = 'review' AND p_card_filter = 'both' THEN
            IF v_learning_due_count > 0 THEN
                SELECT s.word_id, s.mode, 'learning' INTO v_word_id, v_selected_mode, v_source
                FROM user_word_status s
                JOIN word_entries w ON w.id = s.word_id
    ...
    IF v_word_id IS NULL AND p_queue_turn = 'auto' AND p_card_filter = 'both' THEN
        -- Prefer graduated due reviews first
        SELECT s.word_id, s.mode, 'review' INTO v_word_id, v_selected_mode, v_source
        FROM user_word_status s
        JOIN word_entries w ON w.id = s.word_id
        WHERE s.user_id = p_user_id 
    ...
    -- Fallback D: future-due review practice (only if nothing else)
