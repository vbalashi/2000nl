...
-- Implements distinct "new" and "review" queues with round-robin rotation
-- Learning graduation: when interval reaches 1+ day

-- 1. Add new_review_ratio setting (show N reviews per 1 new card)
ALTER TABLE user_settings 
    ADD COLUMN IF NOT EXISTS new_review_ratio int DEFAULT 2;

-- 2. Redesigned get_next_word with queue-based selection
...
BEGIN
    ...
    -- Count words in REVIEW_POOL (graduated with interval >= 1 day)
    ...
    IF v_word_id IS NULL AND (p_queue_turn = 'review' OR p_card_filter = 'review') AND p_card_filter != 'new' THEN
        ...
        -- B1: Pick from review pool (graduated cards, sorted by most forgotten/overdue)
        ...
        ORDER BY s.next_review_at ASC  -- Most overdue/forgotten first
        LIMIT 1;
    END IF;
...
-- 3. Backward-compatible single-mode overload
CREATE OR REPLACE FUNCTION get_next_word(
    p_user_id uuid,
    p_mode text,
    p_exclude_ids uuid[] DEFAULT ARRAY[]::uuid[],
