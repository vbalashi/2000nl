-- Optimize gated dictionary search after live post-migration QA found
-- statement timeouts on ordinary production queries such as "huis".

CREATE INDEX IF NOT EXISTS word_entries_language_lower_headword_idx
    ON word_entries(language_code, lower(headword), dictionary_id);

CREATE INDEX IF NOT EXISTS word_forms_language_lower_form_dictionary_idx
    ON word_forms(language_code, lower(form), dictionary_id, word_id);
