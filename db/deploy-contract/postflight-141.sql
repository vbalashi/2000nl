-- Verify the shared v2 scheduler and retain cached-client public RPC adapters.
-- The adapters preserve only the immediately preceding public shapes; private
-- v1 scheduler bodies and obsolete bypass names remain prohibited.
BEGIN;
...
DECLARE
  ...
  cached_direct_definition text;
  cached_filtered_definition text;
BEGIN
  IF shared_selector IS NULL
     ...
     OR to_regprocedure('public.get_next_card(uuid,text[],uuid[],uuid,text,text,text,text[])') IS NULL
     OR to_regprocedure('public.get_next_filtered_card(uuid,text[],uuid[],uuid,text,text,text,text[],jsonb,boolean)') IS NULL
     OR to_regprocedure('public.get_next_filtered_card(uuid,text[],uuid[],uuid,text,text,text,text[],jsonb)') IS NULL
  ...
  IF to_regprocedure('public.get_next_card_without_known(uuid,text[],uuid[],uuid,text,text,text,text[])') IS NOT NULL
    ...
    RAISE EXCEPTION 'db-contract-gate: postflight-failed obsolete-scheduler-bypass-overloads';
  END IF;
  ...
  cached_direct_definition := upper(pg_get_functiondef(
    'public.get_next_card(uuid,text[],uuid[],uuid,text,text,text,text[])'::regprocedure
  ));
  cached_filtered_definition := upper(pg_get_functiondef(
    'public.get_next_filtered_card(uuid,text[],uuid[],uuid,text,text,text,text[],jsonb)'::regprocedure
  ));
  ...
  IF position('GET_NEXT_CARD(' IN cached_direct_definition) = 0
     OR position('FALSE' IN cached_direct_definition) = 0
     OR position('GET_NEXT_FILTERED_CARD(' IN cached_filtered_definition) = 0
     OR position('FALSE' IN cached_filtered_definition) = 0
     OR position('TRAINING_SCHEDULER_CANDIDATES_V1' IN cached_direct_definition) > 0
     OR position('TRAINING_SCHEDULER_CANDIDATES_V1' IN cached_filtered_definition) > 0 THEN
    RAISE EXCEPTION 'db-contract-gate: postflight-failed cached-client-scheduler-compatibility';
  END IF;
