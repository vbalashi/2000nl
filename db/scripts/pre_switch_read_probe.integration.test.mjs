...
test(
  ...
  () => {
    ...
    if (
      ...
      !/(contract_test|issue238|issue243|issue353|issue355)/i.test(localTarget.pathname)
    ) {
    ...
    assert.match(first.stdout, /compatible 2000nl-db-141/);

    const replay = apply(containerTarget.toString());
    assert.equal(replay.status, 0, replay.stderr);
    assert.match(replay.stdout, /no-op 141/);
    assert.match(replay.stdout, /pre-switch-read-probe passed/);
    assert.match(replay.stdout, /compatible 2000nl-db-141/);
